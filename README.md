...
### Что сильнее: иерархия сигналов

Если разложить, что именно двигает прогноз, порядок на этих данных получается такой.

1. **Личная история.** Доля прошлых заказов покупателя, где был отдел. Одна эта таблица даёт
   hit@3 = 0,620, и никакой другой сигнал к ней почти ничего не добавляет. Люди в продуктовом
   очень постоянны: кто брал молочное в 17 заказах из 20, возьмёт и в 21-м.
2. **Общая популярность отделов.** Что берут все покупатели: hit@3 = 0,506 сама по себе. Это
   фон, на котором работает всё остальное, и единственная опора для покупателя без истории.
3. **Текущий отдел**, откуда человек только что положил товар. Марковская цепь по нему
   прибавляет к популярности около 1,2 пункта (0,518).
4. **Состав корзины целиком.** Ещё 1–2 пункта поверх популярности, и только после нормировки
   на то, что уже лежит в корзине. Без неё сигнал от корзины хуже, чем ничего.

Разрыв между первым пунктом и остальными большой: история даёт 0,62, а корзина с текущим
отделом поверх популярности вместе около 0,52. Причина в самих данных: на уровне 19 отделов
покупки слабо связаны, овощи и молочное есть почти в каждом заказе независимо от остального.
На уровне полок связи в разы сильнее, поэтому следующий шаг для этого проекта: предсказывать
не отдел, а полку. CatBoost, который видит все четыре сигнала сразу плюс время и номер заказа,
обходит историю на 0,7 пункта, и это как раз вклад всего, что не сводится к привычке.
