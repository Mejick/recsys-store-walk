...
| Формула демо: hazard-таблицы + история | 0,314 | 0,619 | 0,512 |
...
- Самая полезная находка про корзину: наивное P(следующий = L | в корзине есть c) врёт.
  ...
  Она даёт 0,619 hit@3, то есть уровень персональной популярности, но при этом реагирует на
  корзину и работает для покупателя без истории.
