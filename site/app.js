/* Магазин-бродилка: демо на реальных агрегатах Instacart.
   ...
   ln P(L | L ещё нет в корзине) + Σ по отделам корзины (доля × ln lift корзины) × 2 + ln lift перехода
   от текущего отдела + 4·доля прошлых заказов с отделом L. Таблицы лежат в site/data и совпадают с results/. */
(function () {
  "use strict";
  var $ = function (id) { return document.getElementById(id); };
  var W_BASKET = 2, W_ROOM = 1, W_HIST = 4;
  ...
  function tables() {
    ...
    $("howto").innerHTML = "<p>Все числа посчитаны по " + META.n_orders.toLocaleString("ru-RU") + " реальным заказам " + META.n_users.toLocaleString("ru-RU") + " покупателей Instacart (Kaggle), отделы <code>missing</code> и <code>other</code> исключены. " +
      ...
      "<p>Оценка отдела L: ln P(L следующий | L ещё нет) + 2·сумма по отделам корзины (доля товаров × ln, во сколько раз чаще идут в L, если этот отдел уже в корзине) + ln того же для текущего отдела + 4·доля ваших прошлых заказов с L. Отделы из корзины и «Для животных» не предлагаются. " +
      "На тесте эта формула даёт hit@3 = 0,617 против 0,627 у CatBoost и 0,506 у простой популярности, таблица в README репозитория.</p>";
  }
